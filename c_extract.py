"""
Usage:
  c_extract.py [--compact] <input> [-- <clang_args>...]
    ...
    def process(self, cursor):
        if cursor.kind == clang.CursorKind.VAR_DECL:
            ...
            struct = dict(
                kind='struct',
                name=cursor.spelling,
                fields=[(f.type.spelling, f.spelling) for f in cursor.type.get_fields()],
                verbatim=verbatim_tokens(cursor),
            )
            ...
            function = dict(
                ...
                arguments=[(a.type.spelling, a.spelling) for a in cursor.get_arguments()],
...
def main():
    ...
    print(json.dumps(visitor.defs, indent=None if opts.get('--compact') else 2))
